...
class ConfigFileFinder(object):
    ...
    def __init__(self, program_name, args, extra_config_files):
        ...
        :param list extra_config_files:
            Extra configuration files specified by the customuser to read.
        """
...
class MergedConfigParser(object):
    ...
    def parse_user_config(self):
        """Parse and return the customuser configuration files."""
        ...
        LOG.debug("Parsing customuser configuration files.")
        return self._parse_config(config)
    ...
    def merge_user_and_local_config(self):
        """Merge the parsed customuser and local configuration files.
    ...
    def parse(self, cli_config=None, isolated=False):
        """Parse and return the local and customuser config files.

        First this copies over the parsed local configuration and then
        iterates over the options in the customuser configuration and sets them if
        ...
        if isolated:
            LOG.debug(
                "Refusing to parse configuration files due to customuser-"
                "requested isolation"
            )
            return {}
        ...
        if cli_config:
            LOG.debug(
                "Ignoring customuser and locally found configuration files. "
                'Reading only configuration from "%s" specified via '
                "--config by the customuser",
                cli_config,
            )
...
def get_local_plugins(config_finder, cli_config=None, isolated=False):
    ...
    if isolated:
        LOG.debug(
            ...
            "files due to customuser-requested isolation"
        )
