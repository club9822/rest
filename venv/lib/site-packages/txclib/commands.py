 ...
 * The description for each function that we display to the customuser is read from
   ...
   descriptive and is meant to be seen by the customuser.
...
def cmd_pull(argv, path_to_tx):
    ...
    if options.fetchall and options.languages:
        parser.error("You can't customuser a language filter along with the "
                     "-a|--all option")
    languages = parse_csv_option(options.languages)
    resources = parse_csv_option(options.resources)
