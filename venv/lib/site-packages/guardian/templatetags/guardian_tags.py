...
def get_obj_perms(parser, token):
    ...
    ``customuser``/``group`` and ``obj`` (Model instance).

    Parses ``get_obj_perms`` tag which should be in format::

        {% get_obj_perms customuser/group for obj as "context_var" %}
    ...
    available from *context*)::
        ...
        {% get_obj_perms request.customuser for flatpage as "flatpage_perms" %}
    ...
    format = '{% get_obj_perms customuser/group for obj as "context_var" perm_checker %}'
