...
def permission_required(perm, lookup_variables=None, **kwargs):
    """
    Decorator for views that checks whether a customuser has a particular permission
    enabled.
    ...
    check permissions on it (without this, one would need to fetch customuser instance
    at view's logic and check permission inside a view).

    :param login_url: if denied, customuser would be redirected to location set by
    ...
    :param accept_global_perms: if set to ``True``, then *object level
      permission* would be required **only if customuser does NOT have global
    ...
    Examples::
        ...
        def my_view(request, username):
            ...
            '''
            customuser = get_object_or_404(User, username=username)
            return customuser.get_absolute_url()
        ...
        def my_view(request, username, group_name):
            ...
            one of customuser's group is named same as request's ``group_name`` param.
            '''
            customuser = get_object_or_404(User, username=username,
...
def permission_required_or_403(perm, *args, **kwargs):
    ...
    Standard Django's permission_required decorator redirects customuser to login page
