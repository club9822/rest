...
class AssignPermTest(ObjectPermissionTestCase):
    """
    Tests permission assigning for customuser/group and object.
    """
...
class GetObjectsForUser(TestCase):
    ...
    def test_non_integer_primary_key_with_group_values(self):
        """
        Verify that the function works when the objects that should be returned
        have non-integer primary keys, and those objects are due to the customuser's
        groups.
        """
        obj_with_char_pk = NonIntPKModel.objects.create(
            char_pk='testprimarykey')
