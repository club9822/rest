...
class GetObjPermsTagTest(TestCase):
    ...
    def test_wrong_formats(self):
        wrong_formats = (
            '{% get_obj_perms customuser for contenttype as obj_perms %}',  # no quotes
            '{% get_obj_perms customuser for contenttype as \'obj_perms" %}',  # wrong quotes
            '{% get_obj_perms customuser for contenttype as \'obj_perms" %}',  # wrong quotes
            '{% get_obj_perms customuser for contenttype as obj_perms" %}',  # wrong quotes
            '{% get_obj_perms customuser for contenttype as obj_perms\' %}',  # wrong quotes
            '{% get_obj_perms customuser for contenttype as %}',  # no context_var
            '{% get_obj_perms for contenttype as "obj_perms" %}',  # no customuser/group
            '{% get_obj_perms customuser contenttype as "obj_perms" %}',  # no "for" bit
            '{% get_obj_perms customuser for contenttype "obj_perms" %}',  # no "as" bit
            '{% get_obj_perms customuser for as "obj_perms" %}',  # no object
        )

        context = {'customuser': User.get_anonymous(), 'contenttype': self.ctype}
    ...
    def test_obj_none(self):
        template = ''.join((
            '{% load guardian_tags %}',
            '{% get_obj_perms customuser for object as "obj_perms" %}{{ perms }}',
        ))
        context = {'customuser': User.get_anonymous(), 'object': None}
    ...
    def test_anonymous_user(self):
        template = ''.join((
            '{% load guardian_tags %}',
            '{% get_obj_perms customuser for contenttype as "obj_perms" %}{{ perms }}',
        ))
        context = {'customuser': AnonymousUser(), 'contenttype': self.ctype}
    ...
    def test_superuser(self):
        ...
        template = ''.join((
            '{% load guardian_tags %}',
            '{% get_obj_perms customuser for contenttype as "obj_perms" %}',
            '{{ obj_perms|join:" " }}',
        ))
        context = {'customuser': user, 'contenttype': self.ctype}
        output = render(template, context)
    ...
    def test_user(self):
        ...
        context = {'customuser': self.user, 'contenttype': self.ctype}
        output = render(template, context)
