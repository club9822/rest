...
class LoginRequiredMixin:
    ...
    The mixin has exactly the same flow as `login_required` decorator:
        ...
        If the customuser isn't logged in, redirect to ``settings.LOGIN_URL``, passing
        the current absolute path in the query string. Example:
        ``/accounts/login/?next=/polls/3/``.

        If the customuser is logged in, execute the view normally. The view code is
        free to assume the customuser is logged in.
...
class PermissionRequiredMixin:
    """
    A view mixin that verifies if the current logged in customuser has the specified
    permission by wrapping the ``request.customuser.has_perm(..)`` method.
    ...
    The mixin does the following:
        ...
        If the customuser is logged in, and passes the permission check than the view
    ...
    **Class Settings**
    ...
    def get_required_permissions(self, request=None):
        ...
        should be checked against *request.customuser* and *object*. By default, it
    ...
    def check_permissions(self, request):
        """
        Checks if *request.customuser* has all permissions returned by
