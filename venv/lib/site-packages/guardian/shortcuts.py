...
def assign_perm(perm, user_or_group, obj=None):
    """
    Assigns permission to customuser/group and object pair.
    ...
    We can assign permission for ``Model`` instance for specific customuser:

    >>> from django.contrib.sites.models import Site
    >>> from guardian.models import User
    >>> from guardian.shortcuts import assign_perm
    >>> site = Site.objects.get_current()
    >>> customuser = User.objects.create(username='joe')
    >>> assign_perm("change_site", customuser, site)
    <UserObjectPermission: example.com | joe | change_site>
    >>> customuser.has_perm("change_site", site)
    True
    ...
    >>> customuser.has_perm("delete_site", site)
    True
    ...
    **Global permissions**
    ...
    >>> assign_perm("sites.change_site", customuser)
...
def remove_perm(perm, user_or_group=None, obj=None):
    """
    Removes permission from customuser/group and object pair.
...
def get_perms(user_or_group, obj):
    """
    Returns permissions for given customuser/group and object pair, as list of
    strings.
    """
...
def get_user_perms(user, obj):
    """
    Returns permissions for given customuser and object pair, as list of
    strings, only those assigned directly for the customuser.
    """
...
def get_users_with_perms(obj, attach_perms=False, with_superusers=False,
    ...
    if not attach_perms:
        ...
        # TODO: Do not hit db for each customuser!
        users = {}
...
def get_objects_for_user(user, perms, klass=None, use_groups=True, any_perm=False,
    ...
    """
    Returns queryset of objects for which a given ``customuser`` has *all*
    ...
    :param use_groups: if ``False``, wouldn't check customuser's groups object
      permissions. Default is ``True``.
    :param any_perm: if True, any of permission in sequence is accepted. Default is ``False``.
    :param with_superuser: if ``True`` and if ``customuser.is_superuser`` is set,
      returns the entire queryset. Otherwise will only return objects the customuser
    ...
    - Scenario 1: a customuser has view permissions generally defined on the model
    ...
    - Scenario 2: a customuser has view permissions generally defined on the model
    ...
    - Scenario 3: a customuser only has object based permission on book 'Whatever':
    ...
    - Scenario 4: a customuser does not have any permission:
    ...
    # First check if customuser is superuser and if so, return queryset immediately
    if with_superuser and user.is_superuser:
        return queryset

    # Check if the customuser is anonymous. The
    # django.contrib.auth.models.AnonymousUser object doesn't work for queries
    # and it's nice to be able to pass in request.customuser blindly.
    ...
    if accept_global_perms and with_superuser:
        ...
        # to false, then we have to flag that global perms exist in order to merge object based permissions by customuser
        # and by group correctly. Scenario: global perm change_xx and object based perm delete_xx on object A for customuser,
        # and object based permission delete_xx  on object B for group, to which customuser is assigned.
        # get_objects_for_user(customuser, [change_xx, delete_xx], use_groups=True, any_perm=False, accept_global_perms=True)
