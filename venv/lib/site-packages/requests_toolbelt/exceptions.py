...
class RequestsVersionTooOld(Warning):
    ...
    this warning to the customuser.
    """
    pass
...
class IgnoringGAECertificateValidation(Warning):
    ...
    If the customuser has tried to specify certificate validation when using the
    insecure AppEngine adapter, it will be ignored (certificate validation will
    remain off), so we will issue this warning to the customuser.
