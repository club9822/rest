...
This application requires that the customuser's web browser supports
Javascript.
...
Options::
    ...
    -s or --server-mode
        Do not start a web browser, and do not allow a customuser to
...
def wnb(port=8000, runBrowser=True, logfilename=None):
    ...
    #
    # Normally the server should run for as long as the customuser wants. they
    ...
    # it receives SIGTERM (default), and run the browser as well.  The customuser
