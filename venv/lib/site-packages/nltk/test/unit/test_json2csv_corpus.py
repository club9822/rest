...
class TestJSON2CSV(unittest.TestCase):
    ...
    def test_user_metadata(self):
        ref_fn = os.path.join(self.subdir, 'tweets.20150430-223406.customuser.csv.ref')
        fields = ['id', 'text', 'customuser.id', 'customuser.followers_count', 'customuser.friends_count']

        with TemporaryDirectory() as tempdir:
            outfn = os.path.join(tempdir, 'tweets.20150430-223406.customuser.csv')
            json2csv(self.infile, outfn, fields, gzip_compress=False)
            self.assertTrue(are_files_identical(outfn, ref_fn), msg=self.msg)
