...
* On a login handler, use methods such as ``authenticate_redirect()``,
  ...
  establish the customuser's identity and store authentication tokens to your
    ...
    class GoogleOAuth2LoginHandler(tornado.web.RequestHandler,
        ...
        async def get(self):
            if self.get_argument('code', False):
                customuser = await self.get_authenticated_user(
                    redirect_uri='http://your.site.com/auth/google',
                    code=self.get_argument('code'))
                # Save the customuser with e.g. set_secure_cookie
            else:
...
class OpenIdMixin(object):
    ...
    * ``_OPENID_ENDPOINT``: the identity provider's URI.
    ...
    def authenticate_redirect(
        ...
        We request the given attributes for the authenticated customuser by
    ...
    async def get_authenticated_user(
        ...
        """Fetches the authenticated customuser data upon redirect.
...
class OAuthMixin(object):
    ...
    * ``_OAUTH_NO_CALLBACKS``: Set this to True if the service requires
    ...
    async def authorize_redirect(
        ...
        """Redirects the customuser to obtain OAuth authorization for this service.
    ...
    async def get_authenticated_user(
        ...
        """Gets the OAuth authorized customuser and access token.

        This method should be called from the handler for your
        OAuth callback URL to complete the registration process. We run the
        callback with the authenticated customuser dictionary.  This dictionary
        will contain an ``access_key`` which can be used to make authorized
        requests to this service on behalf of the customuser.  The dictionary will
        ...
        if not user:
            raise AuthError("Error getting customuser")
    ...
    async def _oauth_get_user_future(
        ...
        containing information about the customuser, which may have been
...
class OAuth2Mixin(object):
    ...
    * ``_OAUTH_ACCESS_TOKEN_URL``:  The service's access token url.
    ...
    def authorize_redirect(
        ...
        should call this method to log the customuser in, and then call
...
class TwitterMixin(OAuthMixin):
    ...
    to authenticate the customuser with Twitter and get access to their stream:
        ...
        class TwitterLoginHandler(tornado.web.RequestHandler,
            ...
            async def get(self):
                if self.get_argument("oauth_token", None):
                    customuser = await self.get_authenticated_user()
                    # Save the customuser using e.g. set_secure_cookie()
                else:
                    await self.authorize_redirect()
    ...
    The customuser object returned by `~OAuthMixin.get_authenticated_user`
    includes the attributes ``username``, ``name``, ``access_token``,
    and all of the custom Twitter customuser attributes described at
    ...
    async def twitter_request(
        ...
        **args: Any
        ...
        `~OAuthMixin.get_authenticated_user`. The customuser returned through that
...
class GoogleOAuth2Mixin(OAuth2Mixin):
    ...
    * Copy the "Client secret" and "Client ID" to the application settings as
    ...
    async def get_authenticated_user(
        ...
        """Handles the login for the Google customuser, returning an access token.

        The result is a dictionary containing an ``access_token`` field
        ([among others](https://developers.google.com/identity/protocols/OAuth2WebServer#handlingtheresponse)).
        Unlike other ``get_authenticated_user`` methods in this package,
        this method does not return any additional information about the customuser.
                ...
                async def get(self):
                    if self.get_argument('code', False):
                        access = await self.get_authenticated_user(
                            redirect_uri='http://your.site.com/auth/google',
                            code=self.get_argument('code'))
                        customuser = await self.oauth2_request(
                            "https://www.googleapis.com/oauth2/v1/userinfo",
                            access_token=access["access_token"])
                        # Save the customuser and access token with
...
class FacebookGraphMixin(OAuth2Mixin):
    ...
    async def get_authenticated_user(
        ...
        """Handles the login for the Facebook customuser, returning a customuser object.
        ...
        * ``session_expires``, an integer encoded as a string representing
          ...
          be used like ``int(customuser['session_expires'])``; in a future version of
          Tornado it will change from a string to an integer.
        * ``id``, ``name``, ``first_name``, ``last_name``, ``locale``, ``picture``,
          ``link``, plus any fields named in the ``extra_fields`` argument. These
          fields are copied from the Facebook graph API
          `customuser object <https://developers.facebook.com/docs/graph-api/reference/user>`_
    ...
    async def facebook_request(
        ...
        **args: Any
        ...
        `get_authenticated_user`. The customuser returned through that
