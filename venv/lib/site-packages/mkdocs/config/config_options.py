        ...
        if '.' not in self.moved_to:
            ...
            for key in move_to.split('.'):
                ...
                if not isinstance(target, dict):
                    # We can't move it for the customuser
                    return
...
class Private(OptionallyRequired):
    ...
    A config option only for internal use. Raises an error if set by the customuser.
    """
...
class MarkdownExtensions(OptionallyRequired):
    ...
    overriden by the customuser. However, builtins can be duplicated to define
